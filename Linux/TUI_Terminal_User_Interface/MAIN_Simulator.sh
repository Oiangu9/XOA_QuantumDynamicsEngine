...
until [[ $option -eq Out ]]; do
    ...
    read xjmax yjmax
    echo "Do you wish to use the adibatic states in y sections? 1 is yes, 0 is no"
    read trash
    read b_y
    ...
    if [[ $gif == *"G"* ]]; then
        ...
        gnuplot -e "set terminal gif size 1800,900 animate delay 1; set output '../ANIMATION_GIFS/$expLabel XO_2D_AdvKin_CorrelPot_$currentDateTime  b_y($b_y) x,yjmax($xjmax,$yjmax) x,yDivs($xDivs,$yDivs) x,ymin($xmin,$ymin) x,ymax($xmax,$ymax) nTrjs($numTrajs) dt($dt) tIt($tIt) mx,y($massx,$massy) outEvry($outputEvery).gif'; t=0; countTrajs=0; tmax=2*$tIt*$numTrajs/$outputEvery; dx1=($xmax-($xmin))/$xDivs; dx2=($ymax-($ymin))/$yDivs; array posx1[$xDivs+1]; array posx2[$yDivs+1]; do for [i=1:($xDivs+1)] { posx1[i] = $xmin + i*dx1 }; do for [i=1:($yDivs+1)] { posx2[i] = $ymin + i*dx2 }; while(t<tmax){  set multiplot; set origin 0.0, 0.0; set size 0.5, 0.5; clear; set xrange [$xmin:$xmax]; set yrange [0:0.12]; set xlabel 'Position x'; set ylabel 'Probab Density';plot 'DATA_probabilityToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP.txt' index t using (posx1[\$0+1]):1 title 'x Conditional Wave Function Probability Density' w l, 'DATA_trajectoriesToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP.txt' every ::(t/2)::(t/2+1) using 1:3 title 'x component of the Trajectory' w points lt 5 pt 6 ps 2 lc rgb 'red';
        set origin 0.5, 0.5; set size 0.5, 0.5; clear; set xrange [$ymin:$ymax]; set yrange [0:0.12]; set xlabel 'Position y'; set ylabel 'Probab Density'; plot 'DATA_probabilityToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP.txt' index (t+1) using (posx2[\$0+1]):1 title 'y Conditional Wave Function Probability Density' w l, 'DATA_trajectoriesToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP.txt' every ::(t/2)::(t/2+1) using 2:3 title 'y component of the Trajectory' w points lt 5 pt 6 ps 2 lc rgb 'red';
        set origin 0.0, 0.5; set size 0.5, 0.5; clear; set xrange [$xmin:$xmax]; set yrange [$ymin:$ymax]; set xlabel 'Position x'; set ylabel 'Position y'; if ((t-(2*$tIt*countTrajs)) >= 2*$tIt){ countTrajs=countTrajs+1; }; set pm3d map; set palette rgbformulae 33,13,10; splot 'DATA_potentialToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP.txt' index ((t-(2*$tIt*countTrajs))/2) title 'Potential Energy Map', 'DATA_trajectoriesToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP.txt' title 'Resultant Trajectories' w points lt 5 pt 4 ps 0.2 lc rgb 'black', 'DATA_trajectoriesToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP.txt' every ::1::(t/2+1) title 'Trajectories' w points lt 5 pt 4 ps 0.2 lc rgb 'red';
        t=t+2*$speedUp; unset multiplot; }"
