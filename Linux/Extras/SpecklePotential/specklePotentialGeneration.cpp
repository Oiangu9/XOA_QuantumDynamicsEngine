/* Script to make trials for the plot of a Speckle Random Potential
 ...
 f) Using the grid discretiçed for the simulation we build the potential field
...
int main(){
    ...
    double VR=2.4, Vmin=-VR, Vmax=4*VR;
    int nV=3000;
    ...
    double xmin=-35.0, xmax=35.0, grainSigma = 0.5;
    ...
    SplineFunction spline(posGrain, potentialGrain);
    
    /* // pa ver que las spline interpolan como dios manda, tenian el problema que la interpollacion solo se hacia de t en  [0,1] asi que habia que escalar el intervalo considerado de las x del soporte a este
     
    for(int i=0; i<=nGrainIntervals; ++i){
        cout << "Should be sirou: " << potentialGrain(i) << " - " << spline(posGrain(i)) << " = " << potentialGrain(i)-spline(posGrain(i)) << endl;
    }
    */
