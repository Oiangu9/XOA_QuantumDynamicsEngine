...
int nx1=100,nx2=100, numIt=83, numTrajs=5, gridPoints, aux, wholex1, wholex2;
double x1min=-30.0,x1max=16.0,x2min=-26.0,x2max=26.0,xBound=0.0,dx1,dx2, normWF, sumCorners, sumBorders, sumInterior, sumaParaChisx, hbar=1.0, m1=1.0, m2=1.0, fractionalx1, fractionalx2, wholex1f, wholex2f, dt=.120;
double xmin=-30.0,xmax=16.0,ymin=-26.0,ymax=26.0;
...
cdouble EigenstatesForSectionsInx(double y, double x, int j){double d=7.0, L, o;if(x<0){ L=ymax-ymin; o=ymin;} else{ if(abs(y)>d){return 0.0;} L=2.0*d; o=-d; } return sqrt(2.0/L)*sin(j*PI*(y-o)/L);}
...
int main(){
inputFileWF.open("/home/oiangu/Desktop/XOA_QuantumDynamicsEngine/ResearchLab/QUANTUM_CAT_Project/KineticAdvective_Correlation_Algorithm_LAB/Kinetic_Advective_Full_CN_XOKA_Comparison/SOURCE_FILES/DATA_rawSimulationData_2D_CN.txt"); //the path of the file where the WF is saved in a sequentialized matrix vector fashion
string line;
cdouble arrayEl;
...
trajFile.open("DATA_CN_Trajs_k=0.1.txt");
...
trajProps.open("DATA_CN_trajProps_k=0.1.txt");
areaProps.open("DATA_CN_areaProps_k=0.1.txt");
