...
psiIni="double sigmax=${sigmax}, sigmay=${sigmay}, mux=${mux}, muy=${muy}, kx=${k0}, ky=0; return pow(1.0/(2*PI*sigmax*sigmax),0.25)*exp(J*kx*x-0.25*pow((x-mux)/sigmax,2))* pow(1.0/(2*PI*sigmay*sigmay),0.25)*exp(J*ky*y-0.25*pow((y-muy)/sigmay,2));"
    ...
    START_TIME=$SECONDS
        ...
        set origin -0.007,0.0; set size 0.25, 0.33; clear; set key title 'R^4 G_x(x,y,t) for this trajectory KA y(t)'; set palette rgbformulae 33,13,10; set pm3d map; set xrange [$x1min:$x1max]; set yrange [$x2min:$x2max]; set xlabel 'Position x'; set ylabel 'Position y'; set cbrange[-2e-7<*:*<1e-6]; set colorbox; splot 'DATA_Full_WF_G_J_CN.txt' index t using 1:2:3 title 'Full WF R^4 G_x(x,y,t)'; unset pm3d; unset view; unset colorbox; unset key;
        ...
        set origin 0.25,0.0; set size 0.25, 0.33; clear;  set xtics auto; set key title 'R^4 J_x(x,y,t) for this trajectory KA y(t)' at 1,39; set palette rgbformulae 33,13,10; set pm3d map; set xrange [$x1min:$x1max]; set yrange [$x2min:$x2max]; set xlabel 'Position x'; set ylabel 'Position y'; set cbrange[-2e-7<*:*<1.5e-7]; set colorbox; splot 'DATA_Full_WF_G_J_CN.txt' index t using 1:2:4 title 'Full WF R^4 J_x(x,y,t)'; unset pm3d; unset view; unset colorbox; unset key;
        set origin 0.5, 0.66; set size 0.25, 0.33; clear; set xtics auto; set xrange [$x1min:$x1max]; set autoscale y; set key title 'x Conditional Wave Function Probability Density PLOT RENORMALIZED' inside; set xlabel 'Position x'; set ylabel 'Probab Density'; plot 'DATA_probabilityToPlot_2D_XO_KinAdv_BornHuang_tINDEP.txt' index t using (posx1[\$0+1]):1 title columnheader(1) w l, 'DATA_probDensity_conditional_WF_CN.txt' index t using (posx1CN[\$0+1]):1 title columnheader(1) w l, 'DATA_trajectoriesToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP_k=$k0.txt' using 1:3 every ::(t/2)::(t/2+2) title 'x component of the Trajectory' w points lt 5 pt 6 ps 1 lc rgb 'red';
        set origin 0.5, 0.33; set size 0.25, 0.33; clear; set xrange [$x2min:$x2max]; set autoscale y; set key title 'y Conditional Wave Function Probability Density PLOT RENORMALIZED';  set xlabel 'Position y'; set ylabel 'Probab Density'; plot 'DATA_probabilityToPlot_2D_XO_KinAdv_BornHuang_tINDEP.txt' index (t+1) using (posx2[\$0+1]):1 title columnheader(1) w l, 'DATA_probDensity_conditional_WF_CN.txt' index (t+1) using (posx2CN[\$0+1]):1 title columnheader(1) w l, 'DATA_trajectoriesToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP_k=$k0.txt' using 2:3 every ::(t/2)::(t/2+2) title 'y component of the Trajectory' w points lt 5 pt 6 ps 2 lc rgb 'red';
