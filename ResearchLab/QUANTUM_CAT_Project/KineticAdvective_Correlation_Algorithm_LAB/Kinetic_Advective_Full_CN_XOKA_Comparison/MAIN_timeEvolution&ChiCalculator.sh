...
do
    ...
    START_TIME=$SECONDS
        ...
        gnuplot -e "set terminal gif size 1950, 900 font 'Helvetica,10' animate delay 1; set output '../ANIMATION_GIFS/$expLabel CN vs XOKA $currentDateTime jmax($jmax) numTrajs($numTrajs).gif'; t=0; tCN=0; countTrajs=0; tmax=2*$numIt*$numTrajs/$outputEvery; dx1=($x1max-($x1min))/$nx1; dx2=($x2max-($x2min))/$nx2; dx1CN=($x1max-($x1min))/$nx1CN; dx2CN=($x2max-($x2min))/$nx2CN; array posx1[$nx1+1]; array posx2[$nx2+1]; array posx1CN[$nx1CN+1]; array posx2CN[$nx2CN+1]; do for [i=1:($nx1+1)] { posx1[i] = $x1min + i*dx1 }; do for [i=1:($nx2+1)] { posx2[i] = $x2min + i*dx2 }; do for [i=1:($nx1CN+1)] { posx1CN[i] = $x1min + i*dx1CN }; do for [i=1:($nx2CN+1)] { posx2CN[i] = $x2min + i*dx2CN }; while(t<tmax){ set multiplot;
        set origin 0.0, 0.48; set size 0.25, 0.5; clear;  if ((t-(2*$numIt*countTrajs)) >= 2*$numIt){ countTrajs=countTrajs+1; }; set key title 'The heatmap is CN |WF(x,y)|^2' at 1,34; set xtics auto; set palette rgbformulae 33,13,10; set pm3d map; set colorbox; set xrange [$x1min:$x1max]; set yrange [$x2min:$x2max]; set xlabel 'Position x'; set ylabel 'Position y'; splot 'DATA_probDensity_WF_CN.txt' index tCN title columnheader(1), 'DATA_trajectoriesToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP_k=$k0.txt' title 'Resultant KA Trajectories' w points lt 5 pt 4 ps 0.2 lc rgb 'black', 'DATA_trajectoriesToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP_k=$k0.txt' every ::1::(t/2+2) title 'traced KA Trajectories' w points lt 5 pt 4 ps 0.2 lc rgb 'red'; unset pm3d; unset view; unset colorbox; unset key;
        set origin 0, 0.0; set size 0.25, 0.5; clear; set key title ' ' inside; set xrange [$x1min:$x1max]; set yrange [-0.5:0.5]; set xlabel 'Position x'; set ylabel 'Correlation Potentials'; set y2tics nomirror; set y2label 'Ax2'; set y2range [-0.5:0.5]; plot 'DATA_KinAdv_x.txt' index t/2 using (posx1[\$0+1]):1 title 'Re{Kin(x)}' axes x1y1 w l, 'DATA_KinAdv_x.txt' index t/2 using (posx1[\$0+1]):2 title 'Im{Kin(x)}' axes x1y1 w l, 'DATA_KinAdv_x.txt' index t/2 using (posx1[\$0+1]):3 title 'Re{Adv(x)}' axes x1y2 w l, 'DATA_KinAdv_x.txt' index t/2 using (posx1[\$0+1]):4 title 'Im{Adv(x)}' axes x1y2 w l, 'DATA_G_J_x.txt' index t/2 using (posx1[\$0+1]):1 title 'G(x)' axes x1y2 w l lc rgb 'red', 'DATA_G_J_x.txt' index t/2 using (posx1[\$0+1]):2 title 'J(x)' axes x1y2 w l lc rgb 'blue'; unset y2tics; unset y2label;
        ...
        set origin 0.25, 0.0; set size 0.25, 0.5; clear; set xrange [-1:($jmax+2)]; set yrange [0:1.1]; set xtics 1; set xlabel 'jmax'; set ylabel 'integrate(\sum_{j=0}^{j=jmax}(|\chi^j(x)|^2))dx'; set key title; plot 'DATA_sumChiInfo_XO.txt' index t/2 using 1:2 with linespoint lw 3 pt 3 title 'KA', 'DATA_sumChiInfo_XO.txt' index t/2 using 1:2:(sprintf('%f', \$2)) with labels center offset -5.4,-.5 rotate by -45 notitle, 'DATA_sumChiInfo_CN.txt' index tCN using 1:2 with linespoint lw 3 pt 3 title 'exact CN', 'DATA_sumChiInfo_CN.txt' index tCN using 1:2:(sprintf('%f', \$2)) with labels center offset -3.4,-.5 rotate by -45 notitle, 'DATA_sumChiInfo_CN_with_KA_traj.txt' index t/2 using 1:2 with linespoint lw 3 pt 3 title 'Cn w KA trajs', 'DATA_sumChiInfo_CN_with_KA_traj.txt' index t/2 using 1:2:(sprintf('%f', \$2)) with labels center offset -1.4,-.5 rotate by -45 notitle;
        ...
        set origin 0.75, 0.5; set size 0.25, 0.5; clear;  set key title; set xrange [$x1min:$x1max]; set yrange [-5:10]; set xlabel 'Position x'; set ylabel 'Re{U^j(x,t)}'; plot for [i=2:($jmax+1)] 'DATA_XO_Re_Uj.txt' index t/2 using (posx1[\$0+1]):i title sprintf('KA Re{U^{%d}(x,t)}',i-1) w l, for [i=2:($jmax+1)] 'DATA_CN_Re_Uj.txt' index t/2 using (posx1CN[\$0+1]):i title sprintf('CN Re{U^{%d}(x,t)}',i-1) w l;
        set origin 0.75, 0.0; set size 0.25, 0.5; clear;  set key title; set xrange [$x1min:$x1max]; set yrange [-12:2]; set xlabel 'Position x'; set ylabel 'Im{U^j(x,t)}'; plot for [i=2:($jmax+1)] 'DATA_XO_Im_Uj.txt' index t/2 using (posx1[\$0+1]):i title sprintf('KA Im{U^{%d}(x,t)}',i-1) w l, for [i=2:($jmax+1)] 'DATA_CN_Im_Uj.txt' index t/2 using (posx1CN[\$0+1]):i title sprintf('CN Im{U^{%d}(x,t)}',i-1) w l;
