...
do
    ...
    ./EXE_codeFileGenerator_CN_Conditional_WFs_with_XO_Trajectories "$DATA_traj_XO_File" "$DATA_wf_CN_File" $nx1CN $nx2CN $jmax $x1min $x2min $x1max $x2max $tIts $numTrajs "$functionEigenstates" $chiSumTolerance 1
    ...
    START_TIME=$SECONDS
        ...
        gnuplot -e "set terminal gif size 1950, 900 font 'Helvetica,11' animate delay 1; set output './OUTPUT_IMAGES/$expLabel CN vs XOKA $currentDateTime jmax($jmax) numTrajs($numTrajs).gif'; t=0; tCN=0; countTrajs=0; tmax=2*$numIt*$numTrajs/$outputEvery; dx1=($x1max-($x1min))/$nx1; dx2=($x2max-($x2min))/$nx2; dx1CN=($x1max-($x1min))/$nx1CN; dx2CN=($x2max-($x2min))/$nx2CN; array posx1[$nx1+1]; array posx2[$nx2+1]; array posx1CN[$nx1CN+1]; array posx2CN[$nx2CN+1]; do for [i=1:($nx1+1)] { posx1[i] = $x1min + i*dx1 }; do for [i=1:($nx2+1)] { posx2[i] = $x2min + i*dx2 }; do for [i=1:($nx1CN+1)] { posx1CN[i] = $x1min + i*dx1CN }; do for [i=1:($nx2CN+1)] { posx2CN[i] = $x2min + i*dx2CN }; while(t<tmax){ set multiplot;
        set origin 0.0, 0.5; set size 0.25, 0.5; clear;  if ((t-(2*$numIt*countTrajs)) >= 2*$numIt){ countTrajs=countTrajs+1; }; set key title 'CN |WF(x,y)|^2'; set xtics auto; set palette rgbformulae 33,13,10; set pm3d map; set xrange [$x1min:$x1max]; set yrange [$x2min:$x2max]; set xlabel 'Position x'; set ylabel 'Position y';splot 'DATA_probDensity_WF_CN.txt' index tCN title columnheader(1), 'DATA_trajectoriesToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP_k=$k0.txt' title 'Resultant Trajectories' w points lt 5 pt 4 ps 0.2 lc rgb 'black', 'DATA_trajectoriesToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP_k=$k0.txt' every ::1::(t/2+2) title 'Trajectories' w points lt 5 pt 4 ps 0.2 lc rgb 'red'; unset pm3d; unset view; unset colorbox;
        set origin 0, 0.0; set size 0.25, 0.5; clear; set key title; set xrange [$x1min:$x1max]; set yrange [-0.4:0.4]; set xlabel 'Position x'; set ylabel 'Correlation Potentials'; set y2tics nomirror; set y2label 'Advective Cor.'; set y2range [-0.4:0.4]; plot 'DATA_KinAdv_x.txt' index t/2 using (posx1[\$0+1]):1 title 'Re{Kin(x)}' axes x1y1 w l, 'DATA_KinAdv_x.txt' index t/2 using (posx1[\$0+1]):2 title 'Im{Kin(x)}' axes x1y1 w l, 'DATA_KinAdv_x.txt' index t/2 using (posx1[\$0+1]):3 title 'Re{Adv(x)}' axes x1y2 w l, 'DATA_KinAdv_x.txt' index t/2 using (posx1[\$0+1]):4 title 'Im{Adv(x)}' axes x1y2 w l, 'DATA_G_J_x.txt' index t/2 using (posx1[\$0+1]):1 title 'G(x)' axes x1y2 w l, 'DATA_G_J_x.txt' index t/2 using (posx1[\$0+1]):2 title 'J(x)' axes x1y2 w l; unset y2tics; unset y2label;
        set origin 0.25, 0.5; set size 0.25, 0.5; clear; set key title; set xrange [$x1min:$x1max]; set yrange [0:0.4]; set xlabel 'Position x'; set ylabel '|Chi^j(x)|'; plot for [i=2:($jmax+1)] 'DATA_chiInfo_XO.txt' index t/2 using (posx1[\$0+1]):i title sprintf('KA |Chi^{%d}(x)|',i-1) w l, for [i=2:($jmax+1)] 'DATA_chiInfo_CN.txt' index tCN using 1:(i+1) title sprintf('CN |Chi^{%d}(x)|',i-1) w l, for [i=2:($jmax+1)] 'DATA_chiInfo_CN_w_KA_trajs.txt' index t/2 using (posx1[\$0+1]):i title sprintf('CN with KA traj condit wfs |Chi^{%d}(x)|',i-1) w l;
        set origin 0.25, 0.0; set size 0.25, 0.5; clear; set xrange [-1:($jmax+2)]; set yrange [0:1.1]; set xtics 1; set xlabel 'jmax'; set ylabel 'integrate(\sum_{j=0}^{j=jmax}(|\chi^j(x)|^2))dx'; set key title; plot 'DATA_sumChiInfo_XO.txt' index t/2 using 1:2 with linespoint lw 3 pt 3 title 'KA', 'DATA_sumChiInfo_XO.txt' index t/2 using 1:2:(sprintf('%f', \$2)) with labels center offset -3.4,-.5 rotate by -45 notitle, 'DATA_sumChiInfo_CN.txt' index tCN using 1:2 with linespoint lw 3 pt 3 title 'exact CN', 'DATA_sumChiInfo_CN.txt' index tCN using 1:2:(sprintf('%f', \$2)) with labels center offset -3.4,-.5 rotate by -45 notitle, 'DATA_sumChiInfo_CN_with_KA_traj.txt' index t/2 using 1:2 with linespoint lw 3 pt 3 title 'Cn w KA trajs', 'DATA_sumChiInfo_CN_with_KA_traj.txt' index t/2 using 1:2:(sprintf('%f', \$2)) with labels center offset -3.4,-.5 rotate by -45 notitle;
        set origin 0.5, 0.5; set size 0.25, 0.5; clear; set xtics auto; set xrange [$x1min:$x1max]; set yrange [0:0.01]; set key title 'x Conditional Wave Function Probability Density'; set xlabel 'Position x'; set ylabel 'Probab Density'; plot 'DATA_probabilityToPlot_2D_XO_KinAdv_BornHuang_tINDEP.txt' index t using (posx1[\$0+1]):1 title columnheader(1) w l, 'DATA_probDensity_conditional_WF_CN.txt' index t using (posx1CN[\$0+1]):1 title columnheader(1) w l, 'DATA_trajectoriesToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP_k=$k0.txt' every ::(t/2)::(t/2+2) using 1:3 title 'x component of the Trajectory' w points lt 5 pt 6 ps 1 lc rgb 'red';
        set origin 0.5, 0.0; set size 0.25, 0.5; clear; set xrange [$x2min:$x2max]; set yrange [0:0.01]; set key title 'y Conditional Wave Function Probability Density';  set xlabel 'Position y'; set ylabel 'Probab Density'; plot 'DATA_probabilityToPlot_2D_XO_KinAdv_BornHuang_tINDEP.txt' index (t+1) using (posx2[\$0+1]):1 title columnheader(1) w l, 'DATA_probDensity_conditional_WF_CN.txt' index (t+1) using (posx2CN[\$0+1]):1 title columnheader(1) w l, 'DATA_trajectoriesToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP_k=$k0.txt' every ::(t/2)::(t/2+2) using 2:3 title 'y component of the Trajectory' w points lt 5 pt 6 ps 2 lc rgb 'red';
        set origin 0.75, 0.5; set size 0.25, 0.5; clear;  set key title; set xrange [$x1min:$x1max]; set yrange [-5:10]; set xlabel 'Position x'; set ylabel 'Re{U^j(x,t)}'; plot for [i=2:($jmax+1)] 'DATA_CN_Re_Uj.txt' index t/2 using (posx1CN[\$0+1]):i title sprintf('CN Re{U^{%d}(x,t)}',i-1) w l, for [i=2:($jmax+1)] 'DATA_XO_Re_Uj.txt' index t/2 using (posx1[\$0+1]):i title sprintf('KA Re{U^{%d}(x,t)}',i-1) w l;
        set origin 0.75, 0.0; set size 0.25, 0.5; clear;  set key title; set xrange [$x1min:$x1max]; set yrange [-12:2]; set xlabel 'Position x'; set ylabel 'Im{U^j(x,t)}'; plot for [i=2:($jmax+1)] 'DATA_CN_Im_Uj.txt' index t/2 using (posx1CN[\$0+1]):i title sprintf('CN Im{U^{%d}(x,t)}',i-1) w l, for [i=2:($jmax+1)] 'DATA_XO_Im_Uj.txt' index t/2 using (posx1[\$0+1]):i title sprintf('KA Im{U^{%d}(x,t)}',i-1) w l;
