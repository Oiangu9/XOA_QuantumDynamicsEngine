...
psiIni="double sigmax=${sigmax}, sigmay=${sigmay}, mux=${mux}, muy=${muy}, kx=${k0}, ky=0; return pow(1.0/(2*PI*sigmax*sigmax),0.25)*exp(J*kx*x-0.25*pow((x-mux)/sigmax,2))* pow(1.0/(2*PI*sigmay*sigmay),0.25)*exp(J*ky*y-0.25*pow((y-muy)/sigmay,2));"
    ...
    START_TIME=$SECONDS
        ...
        gnuplot -e "set terminal gif size 2925, 1350 font 'Helvetica,10' animate delay 1; set output '../ANIMATION_GIFS/$expLabel CN vs XOKA $currentDateTime jmax($jmax) numTrajs($numTrajs).gif'; t=0; tCN=0; countTrajs=0; tmax=2*$numIt*$numTrajs/$outputEvery; dx1=($x1max-($x1min))/$nx1; dx2=($x2max-($x2min))/$nx2; dx1CN=($x1max-($x1min))/$nx1CN; dx2CN=($x2max-($x2min))/$nx2CN; array posx1[$nx1+1]; array posx2[$nx2+1]; array posx1CN[$nx1CN+1]; array posx2CN[$nx2CN+1]; do for [i=1:($nx1+1)] { posx1[i] = $x1min + i*dx1 }; do for [i=1:($nx2+1)] { posx2[i] = $x2min + i*dx2 }; do for [i=1:($nx1CN+1)] { posx1CN[i] = $x1min + i*dx1CN }; do for [i=1:($nx2CN+1)] { posx2CN[i] = $x2min + i*dx2CN }; while(t<tmax){ set multiplot;
        set origin 0.0, 0.66; set size 0.167, 0.33; clear; set key title 'CN |WF(x,y)|^2' at 1,40; set xtics auto; set palette rgbformulae 33,13,10; set pm3d map; set xrange [$x1min:$x1max]; set yrange [$x2min:$x2max]; set xlabel 'Position x'; set ylabel 'Position y'; set cbrange[0:0.012]; set colorbox; splot 'DATA_probDensity_WF_CN.txt' index tCN title columnheader(1); unset pm3d; unset view; unset colorbox; unset key;
        set origin 0, 0.33; set size 0.166, 0.33; clear; set key title 'Potential Energy heatmap and KA trajectories'; set xrange [$x1min:$x1max]; set yrange [$x2min:$x2max]; set xlabel 'Position x'; set ylabel 'Position y'; set pm3d map; set palette rgbformulae 33,13,10; set cbrange[0:100]; set colorbox; splot 'DATA_potentialToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP.txt' title 'Potential Energy Map', 'DATA_trajectoriesToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP_k=$k0.txt' title 'Resultant Trajectories' w points lt 5 pt 4 ps 0.2 lc rgb 'black', 'DATA_trajectoriesToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP_k=$k0.txt' every ::1::(t/2+2) title 'traced KA Trajectories' w points lt 5 pt 4 ps 0.2 lc rgb 'red';unset pm3d; unset view; unset colorbox; unset key;
        set origin -0.007,0.0; set size 0.167, 0.33; clear; set key title 'R^4 G_x(x,y,t) for this trajectory KA y(t)'; set palette rgbformulae 33,13,10; set pm3d map; set xrange [$x1min:$x1max]; set yrange [$x2min:$x2max]; set xlabel 'Position x'; set ylabel 'Position y'; set cbrange[-2e-7<*:*<1e-6]; set colorbox; splot 'DATA_Full_WF_G_J_CN.txt' index t/2 using 1:2:3 title 'Full WF R^4 G_x(x,y,t)'; unset pm3d; unset view; unset colorbox; unset key;
        set origin 0.167, 0.66; set size 0.166, 0.33; clear; set key title '' inside; set xrange [$x1min:$x1max]; set yrange [0:0.4]; set xlabel 'Position x'; set ylabel '|Chi^j(x)|'; plot for [i=2:($jmax+1)] 'DATA_chiInfo_XO.txt' index t/2 using (posx1[\$0+1]):i title sprintf('KA |Chi^{%d}(x)|',i-1) w l, for [i=2:($jmax+1)] 'DATA_chiInfo_CN.txt' index tCN using (posx1CN[\$0+1]):i title sprintf('CN |Chi^{%d}(x)|',i-1) w l, for [i=2:($jmax+1)] 'DATA_chiInfo_CN_w_KA_trajs.txt' index t/2 using (posx1CN[\$0+1]):i title sprintf('CN with KA traj condit wfs |Chi^{%d}(x)|',i-1) w l;
        set origin 0.167, 0.33; set size 0.167, 0.33; clear; set xrange [-1:($jmax+2)]; set yrange [0:1.1]; set xtics 1; set xlabel 'jmax'; set ylabel 'integrate(\sum_{j=0}^{j=jmax}(|\chi^j(x)|^2))dx'; set key title; plot 'DATA_sumChiInfo_XO.txt' index t/2 using 1:2 with linespoint lw 3 pt 3 title 'KA', 'DATA_sumChiInfo_XO.txt' index t/2 using 1:2:(sprintf('%f', \$2)) with labels center offset -5.4,-.5 rotate by -45 notitle, 'DATA_sumChiInfo_CN.txt' index tCN using 1:2 with linespoint lw 3 pt 3 title 'exact CN', 'DATA_sumChiInfo_CN.txt' index tCN using 1:2:(sprintf('%f', \$2)) with labels center offset -3.4,-.5 rotate by -45 notitle, 'DATA_sumChiInfo_CN_with_KA_traj.txt' index t/2 using 1:2 with linespoint lw 3 pt 3 title 'Cn w KA trajs', 'DATA_sumChiInfo_CN_with_KA_traj.txt' index t/2 using 1:2:(sprintf('%f', \$2)) with labels center offset -1.4,-.5 rotate by -45 notitle; unset key;
        set origin 0.167,0.0; set size 0.166, 0.33; clear;  set xtics auto; set key title 'R^4 J_x(x,y,t) for this trajectory KA y(t)' at 1,39; set palette rgbformulae 33,13,10; set pm3d map; set xrange [$x1min:$x1max]; set yrange [$x2min:$x2max]; set xlabel 'Position x'; set ylabel 'Position y'; set cbrange[-1e-6<*:*<2e-7]; set colorbox; splot 'DATA_Full_WF_G_J_CN.txt' index t/2 using 1:2:4 title 'Full WF R^4 J_x(x,y,t)'; unset pm3d; unset view; unset colorbox; unset key;
        set origin 0.33, 0.66; set size 0.167, 0.33; clear; set xtics auto; set xrange [$x1min:$x1max]; set yrange [0:0.01]; set key title 'x Conditional Wave Function Probability Density' inside; set xlabel 'Position x'; set ylabel 'Probab Density'; plot 'DATA_probabilityToPlot_2D_XO_KinAdv_BornHuang_tINDEP.txt' index t using (posx1[\$0+1]):1 title columnheader(1) w l, 'DATA_probDensity_conditional_WF_CN.txt' index t using (posx1CN[\$0+1]):1 title columnheader(1) w l, 'DATA_trajectoriesToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP_k=$k0.txt' using 1:3 every ::(t/2)::(t/2+2) title 'x component of the Trajectory' w points lt 5 pt 6 ps 1 lc rgb 'red';
        set origin 0.33, 0.33; set size 0.166, 0.33; clear; set xrange [$x2min:$x2max]; set yrange [0:0.01]; set key title 'y Conditional Wave Function Probability Density';  set xlabel 'Position y'; set ylabel 'Probab Density'; plot 'DATA_probabilityToPlot_2D_XO_KinAdv_BornHuang_tINDEP.txt' index (t+1) using (posx2[\$0+1]):1 title columnheader(1) w l, 'DATA_probDensity_conditional_WF_CN.txt' index (t+1) using (posx2CN[\$0+1]):1 title columnheader(1) w l, 'DATA_trajectoriesToPlot_2D_XO_CN_KinAdv_BornHuang_tINDEP_k=$k0.txt' using 2:3 every ::(t/2)::(t/2+2) title 'y component of the Trajectory' w points lt 5 pt 6 ps 2 lc rgb 'red';
        set origin 0.34, 0.0; set size 0.167, 0.33; clear; set key title ' ' inside; set xrange [$x1min:$x1max]; set xlabel 'Position x'; set ylabel 'G(x)'; set y2tics nomirror; set y2label 'J(x)'; set yrange [-1e-7<*:*<1e-6]; set y2range [-1e-7<*:*<1e-7]; plot 'DATA_G_J_with_CN_and_KA_trajs.txt' index t/2 using (posx1CN[\$0+1]):1 title 'R^4 G(x) CN with KA trajs' axes x1y1 w l lc rgb 'red', 'DATA_G_J_with_CN_and_KA_trajs.txt' index t/2 using (posx1CN[\$0+1]):2 title 'R^4 J(x) CN with KA trajs' axes x1y2 w l lc rgb 'blue'; unset y2tics; unset y2label; unset key;
        set origin 0.5, 0.66; set size 0.166, 0.33; clear;  set key title; set xrange [$x1min:$x1max]; set xlabel 'Position x'; set ylabel 'Re{U^j(x,t)}'; set y2tics nomirror; set y2label 'Im{U^j(x,t)}'; set y2range [-7.5:5.5]; set yrange [-5.5:5.5]; plot for [i=2:($jmax+1)] 'DATA_XO_Re_Uj.txt' index t/2 using (posx1[\$0+1]):i title sprintf('KA Re{U^{%d}(x,t)}',i-1) axes x1y1 w l, for [i=2:($jmax+1)] 'DATA_CN_Re_Uj.txt' index t/2 using (posx1CN[\$0+1]):i title sprintf('CN Re{U^{%d}(x,t)}',i-1) axes x1y1 w l, for [i=2:($jmax+1)] 'DATA_XO_Im_Uj.txt' index t/2 using (posx1[\$0+1]):i title sprintf('KA Im{U^{%d}(x,t)}',i-1) axes x1y2 w l, for [i=2:($jmax+1)] 'DATA_CN_Im_Uj.txt' index t/2 using (posx1CN[\$0+1]):i title sprintf('CN Im{U^{%d}(x,t)}',i-1) axes x1y2 w l; unset y2tics; unset y2label;
        set origin 0.5, 0.33; set size 0.166, 0.33; clear; set key title ' ' inside; set xrange [$x1min:$x1max]; set yrange [-0.5:0.5]; set xlabel 'Position x'; set ylabel 'Kin'; set y2tics nomirror; set y2label 'Adv'; set y2range [-0.5:0.5]; plot 'DATA_KinAdv_x.txt' index t/2 using (posx1[\$0+1]):1 title 'Re{Kin(x)}' axes x1y1 w l, 'DATA_KinAdv_x.txt' index t/2 using (posx1[\$0+1]):2 title 'Im{Kin(x)}' axes x1y1 w l, 'DATA_KinAdv_x.txt' index t/2 using (posx1[\$0+1]):3 title 'Re{Adv(x)}' axes x1y2 w l, 'DATA_KinAdv_x.txt' index t/2 using (posx1[\$0+1]):4 title 'Im{Adv(x)}' axes x1y2 w l; unset y2tics; unset y2label;
        set origin 0.5, 0.0; set size 0.166, 0.33; clear; set key title ' ' inside; set xrange [$x1min:$x1max]; set yrange [-0.07:0.07]; set xlabel 'Position x'; set ylabel 'G(x)'; set y2tics nomirror; set y2label 'J(x)'; set y2range [-0.07:0.07]; plot 'DATA_G_J_x_KA.txt' index t/2 using (posx1[\$0+1]):1 title 'G(x) KA aprox' axes x1y1 w l lc rgb 'red', 'DATA_G_J_x_KA.txt' index t/2 using (posx1[\$0+1]):2 title 'J(x) KA aprox' axes x1y2 w l lc rgb 'blue'; unset y2tics; unset y2label; unset key;
        set origin 0.65,0.66; set size 0.166, 0.33; clear; set xtics auto; set key title 'R^2 dS_{dy}' at 1,39; set palette rgbformulae 33,13,10; set pm3d map; set xrange [$x1min:$x1max]; set yrange [$x2min:$x2max]; set xlabel 'Position x'; set ylabel 'Position y'; set cbrange[-0.01<*:*<0.01]; set colorbox user origin 0.795,0.72 size 0.005,0.23; splot 'DATA_Full_WF_G_J_CN.txt' index t/2 using 1:2:5 title 'Full WF R^2 dS_{dy}'; unset pm3d; unset view; unset colorbox; unset key;
        set origin 0.65,0.33; set size 0.167, 0.33; clear; set xtics auto; set key title 'R^2 d2S_{dy}' at 1,39; set palette rgbformulae 33,13,10; set pm3d map; set xrange [$x1min:$x1max]; set yrange [$x2min:$x2max]; set xlabel 'Position x'; set ylabel 'Position y'; set cbrange[-0.01<*:*<0.01]; set colorbox; splot 'DATA_Full_WF_G_J_CN.txt' index t/2 using 1:2:6 title 'Full WF R^2 d2S_{dy}'; unset pm3d; unset view; unset colorbox; unset key;
        set origin 0.825,0.66; set size 0.166, 0.33; clear; set xtics auto; set key title 'dR_{dy}' at 1,39; set palette rgbformulae 33,13,10; set pm3d map; set xrange [$x1min:$x1max]; set yrange [$x2min:$x2max]; set xlabel 'Position x'; set ylabel 'Position y'; set cbrange[-0.01<*:*<0.01]; set colorbox; splot 'DATA_Full_WF_G_J_CN.txt' index t/2 using 1:2:7 title 'Full WF dR_{dy}'; unset pm3d; unset view; unset colorbox; unset key;
        set origin 0.825,0.33; set size 0.167, 0.33; clear; set xtics auto; set key title 'd2R_{dy}' at 1,39; set palette rgbformulae 33,13,10; set pm3d map; set xrange [$x1min:$x1max]; set yrange [$x2min:$x2max]; set xlabel 'Position x'; set ylabel 'Position y'; set cbrange[-0.01<*:*<0.01]; set colorbox; splot 'DATA_Full_WF_G_J_CN.txt' index t/2 using 1:2:8 title 'Full WF d2R_{dy}'; unset pm3d; unset view; unset colorbox; unset key;
        set origin 0.65,0.0; set size 0.166, 0.33; clear; set xtics auto; set key title 'G_x(x,y,t)' at 1,39; set palette rgbformulae 33,13,10; set pm3d map; set xrange [$x1min:$x1max]; set yrange [$x2min:$x2max]; set xlabel 'Position x'; set ylabel 'Position y'; set cbrange[-0.08<*:*<0.08]; set colorbox; splot 'DATA_Full_WF_G_J_CN.txt' index t/2 using 1:2:9 title 'Full WF G_x(x,y,t)'; unset pm3d; unset view; unset colorbox; unset key;
        set origin 0.825,0.0; set size 0.167, 0.33; clear; set xtics auto; set key title 'J_x(x,y,t)' at 1,39; set palette rgbformulae 33,13,10; set pm3d map; set xrange [$x1min:$x1max]; set yrange [$x2min:$x2max]; set xlabel 'Position x'; set ylabel 'Position y'; set cbrange[-0.08<*:*<0.08]; set colorbox; splot 'DATA_Full_WF_G_J_CN.txt' index t/2 using 1:2:10 title 'Full WF J_x(x,y,t)'; unset pm3d; unset view; unset colorbox; unset key;
